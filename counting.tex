...
\begin{examp}
    Suppose $G = \GL_n(\BC)$, $\nu \in {^{a}\fh^*}$ is regular integral, and $\CO_{\iota} \times \CO_{\iota} \in \bar{\Nil}(\fg)$ is the minimal orbit, where the Young diagram $\iota$ is
    \[
    \begin{ytableau}
        ~&~\\
        ~\\
    ...
    \]
    for $i = 1,2,\cdots,n-1$.
    This implies $\sharp(\Irr_{\nu}(G;\CO_{\iota} \times \CO_{\iota})) = (n-1)^2$.
\end{examp}
